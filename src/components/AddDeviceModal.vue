                ...
                <div v-if="isSensor" class="space-y-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
                  <div>
                    ...
                    <select v-model="form.subtype" 
                      ...
                      <option value="ppm">🌫️ PPM</option>
                      <option value="rs">⚙️ RS</option>
                      <option value="ads">⚡ ADC</option>
...
function handleSubmit() {
  ...
  if (deviceType === 'sensor') {
    ...
    newDevice.unit = form.unit || (form.subtype === 'ppm' ? 'ppm' : form.subtype === 'rs' ? 'Опір' : form.subtype === 'adc' ? 'ADC' : form.subtype === 'temperature' ? '°C' : form.subtype === 'humidity' ? '%' : form.subtype === 'pressure' ? 'hPa' : '')
    newDevice.value = 0
